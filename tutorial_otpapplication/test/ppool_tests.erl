...
pool_run_mfa(Name) ->
    ppool:run_work(Name, [i_am_running, 1, 1, self()]),
    X = receive
...
pool_run_alloc(Name) ->
    {ok, Pid} = ppool:run_work(Name, [i_am_running, 1, 1, self()]),
    X = receive
        {Pid, i_am_running} -> ok
    after 3000 ->
        timeout
...
pool_run_noalloc(Name) ->
    ...
    ppool:run_work(Name, [i_am_running, 300, 1, self()]),
    ppool:run_work(Name, [i_am_running, 300, 1, self()]),
    X = ppool:run_work(Name, [i_am_running, 1, 1, self()]),
    ?_assertEqual(noalloc, X).
    
pool_run_realloc(Name) ->
    %% Init function should have set the limit to 2
    {ok, A} = ppool:run_work(Name, [i_am_running, 500, 1, self()]),
    timer:sleep(100),
    {ok, B} = ppool:run_work(Name, [i_am_running, 500, 1, self()]),
    timer:sleep(600),
    {ok, Pid} = ppool:run_work(Name, [i_am_running, 1, 1, self()]),
    timer:sleep(100),
    L = flush(),
...
test_async_queue(Name) ->
    ...
    ok = ppool:async_queue_work(Name, [i_am_running, 2000, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 2000, 1, self()]),
    noalloc = ppool:run_work(Name, [i_am_running, 2000, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 500, 1, self()]),
    timer:sleep(3500),
    L = flush(),
...
test_sync_queue(Name) ->
    %% Hell yase, two max
    {ok, Pid} = ppool:sync_queue_work(Name, [i_am_running, 200, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 200, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 200, 1, self()]),
    {ok, Pid2} = ppool:sync_queue_work(Name, [i_am_running, 100, 1, self()]),
    timer:sleep(300),
    L = flush(),
    [?_assert(is_pid(Pid)),
     ?_assert(is_pid(Pid2)),
...
test_supervision(Name) ->
    ppool:start_pool(Name, 1, {ppool_nagger, start_link, []}),
    {ok, Pid} = ppool:run_work(Name, [sup, 10000, 100, self()]),
    ppool:stop_pool(Name),
    timer:sleep(100),
...
test_auth_dealloc(Name) ->
    %% Hell yase, two max
    {ok, _Pid} = ppool:sync_queue_work(Name, [i_am_running, 500, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 10000, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 10000, 1, self()]),
    ok = ppool:async_queue_work(Name, [i_am_running, 1, 1, self()]),
    timer:sleep(600),
