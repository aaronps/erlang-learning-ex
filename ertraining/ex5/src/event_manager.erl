...
%%
%% in these client functions, Name reffers to the event_manager name and
%% Handler is the handler's module name
%%
...
swap_handlers(Name, Old, New) -> call(Name, {swap_handlers, Old, New}).
...
handle_msg({swap_handlers, Old, New}, LoopData) ->
  case lists:keyfind(Old, 1, LoopData) of
    false -> { {error, instance}, LoopData };
    {Old, Data} ->
      NewData = New:init(Old:terminate(Data)),
      { ok, lists:keyreplace(Old, 1, LoopData, {New, NewData})}
  end.
